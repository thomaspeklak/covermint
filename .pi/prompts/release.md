...
Perform a full release of the Covermint project. Follow these steps exactly, stopping and reporting any failure before continuing.
...
2. `cargo clippy -- -D warnings` — fail on any warnings
...
Then run `cargo check` to regenerate `Cargo.lock` with the new version.
...
Show the user the tag and confirm the push succeeded. Remind them that GitHub Actions will now build the release binaries automatically.
